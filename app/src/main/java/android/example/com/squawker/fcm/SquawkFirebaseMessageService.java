...
public class SquawkFirebaseMessageService extends FirebaseMessagingService {
    ...
    private static final String DATA_KEY_TEST = "test";
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (data.size() > 0) {
            ...
            // TODO Is there a way to only send this if the app is in the foreground
            sendNotification(data);

            // If the message is a test message, add it to the database, otherwise, trigger a sync
            if (data.containsKey(DATA_KEY_TEST)) {
                // Test Squawk
                insertSquawk(data);
            } else {
                // Sync the server
    ...
    /**
     * Inserts a single squawk into the database; used for test squawks
     *
     * @param data Map which has the message data in it
     */
    private void insertSquawk(Map<String, String> data) {
        // TODO I'm assuming this is okay not in a separate thread, but want to double check
